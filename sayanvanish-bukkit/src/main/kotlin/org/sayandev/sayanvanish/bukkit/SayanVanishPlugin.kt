...
class SayanVanishPlugin : JavaPlugin() {
    ...
    override fun onEnable() {
        ...
        if (SettingsConfig.get().general.proxyMode && storageConfig.method == DatabaseType.SQL && storageConfig.sql.method == SQLConfig.SQLMethod.SQLITE) {
        ...
        if (SettingsConfig.get().general.bstats) {
            Metrics(this, 23914).apply {
                ...
                this.addCustomChart(Metrics.SimplePie("proxied") {
                    if (SettingsConfig.get().general.proxyMode) "On Proxy" else "No Proxy"
                })
