...
object VanishManager : Listener {
    ...
    fun onPostLogin(event: ServerConnectedEvent) {
        ...
        SayanVanishBungeeAPI.getInstance().database.addUser(User.of(player.uniqueId, player.name, player.server.info.name ?: Platform.get().id))
        val user = player.getOrCreateUser()
    }

    fun onDisconnect(event: PlayerDisconnectEvent) {
        val player = event.player ?: return
        SayanVanishBungeeAPI.getInstance().database.removeUser(player.uniqueId)
    }
